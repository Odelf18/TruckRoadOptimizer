    ...
    try:
        ...
        with st.spinner("🧮 Optimizing with multiple vehicles..."):
            ...
            for num_vehicles in range(min_vehicles_capacity, max_vehicles + 1):
                st.write(f"🔍 Trying with {num_vehicles} vehicle(s)...")
                ...
                try:
                    ...
                    try:
                        ...
                        if solution:
                            ...
                            # Check if this solution fits within working hours
                            if max_vehicle_time <= work_duration_minutes:
                                ...
                                st.write(f"❌ {num_vehicles} vehicle(s): Exceeds working hours ({max_vehicle_time:.1f} > {work_duration_minutes} min)")
                        else:
    ...
    try:
            ...
            try:
                ...
                if solution:
                    ...
                    return {
                        ...
                        'fits_work_hours': max_vehicle_time <= work_duration_minutes,
        ...
        with st.spinner("🧪 Testing 1, 2, 3 vehicles in parallel..."):
            ...
            for result in results:
                if result['success']:
                    status = "✅" if result['fits_work_hours'] else "⚠️"
                    st.write(f"{status} **{result['vehicles']} vehicle(s):** {result['max_time']:.1f} min {'(fits work hours)' if result['fits_work_hours'] else '(exceeds work hours)'}")
                else:
