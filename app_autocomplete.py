...
def get_address_suggestions(query, session_key):
    ...
    try:
        ...
        st.session_state.address_suggestions[session_key] = suggestions
        return suggestions
...
# Saisie des stops avec autocomplétion
...
for i in range(n):
    with st.container():
        ...
        with col1:
            ...
            addr_key = f"addr_{i}"
            addr = st.text_input(f"Adresse stop {i+1}", key=addr_key)
            
            # Afficher les suggestions si l'utilisateur tape
            if addr and len(addr) >= 3:
                suggestions = get_address_suggestions(addr, addr_key)
                if suggestions:
                    st.write("**Suggestions :**")
                    for j, suggestion in enumerate(suggestions):
                        if st.button(f"📍 {suggestion['description']}", key=f"suggest_{i}_{j}"):
                            # Remplir le champ avec la suggestion sélectionnée
                            st.session_state[addr_key] = suggestion['description']
                            st.rerun()
...
with st.expander("ℹ️ Aide"):
    ...
    st.write("\n**Comment utiliser :**")
    st.write("1. **Tapez au moins 3 caractères** dans les champs d'adresse")
    st.write("2. **Cliquez sur une suggestion** pour la sélectionner")
